...
- **PRISM（多维 LLM 审稿人基准）**：四维评审质量（Depth / Novelty / Flaw 主次 / Constructiveness），量化 LLM 审稿病症——novelty 过度背书、constructiveness 缺口（只挑刺不给方案）、surface trap（陷在格式/措辞表层）。`critique_self_audit.py` 的三轴自审对应这些 failure-mode。⚠ **铁律 2 实践**：v1 曾写"SEA 过度背书 79% vs 人 59%""TreeReview surface 24%""CycleReviewer 幻觉 18.5%"三具体数字，**2026-06-18 一手 fetch SEA(2407.12857) / TreeReview(2506.07642) 原文均无此数字** → 已剥离，只留可核机制（过度背书 / 肤浅反馈，定性）。过度背书的硬实证改引 **Pitfalls(arXiv 2512.22145)**：LLM 均分 7.5-9.0 vs 人类中位 3-7、系统高估 3-5 分、对弱稿 MAE 飙到 4.5-6.2。
...
【已知坑/诚实，2026-06-18 一手核更新】① **GraphMind F1=0.75**（74% acc/0.78 P/0.72 R，SciNova benchmark）**已一手 fetch arXiv 2510.15706 核实**，可引；citation 立场 supporting/contrasting + background/target 分解机制亦核实。② **SEA 79% / TreeReview 24% / CycleReviewer 18.5% 三数字一手核两篇原文均无 → 已从本技能全部剥离**（见 §14 与 `critique_self_audit.py`）。③ **RND AUROC 0.782** 本环境未联网复核原文，引用标"待核查"；本技能借鉴的是**方法思路**（密度百分位），不依赖该数值成立。④ 密度新颖分依赖 literature-search 检索覆盖，漏掉最像那篇会高估（GIGO）。⑤ 真·多模型异质并行评审本技能**未实现**——单模型扮多视角仍是伪多样性塌缩风险，机检清单只缓解，诚实标注为能力边界而非已做。
