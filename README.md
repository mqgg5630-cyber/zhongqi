...
> 封面信息已从 `sources/开题.docx` 读取并填入（中期检查表封面 7 栏 + 三版 PPT 封面）。
...
| 论文题目 | ✅ | 开题报告题目 |
| 1. 论文研究主要内容及工作进度 | ✅ 31 段（6 个小标题） | 开题报告 + 已完成1.docx：选题背景与意义、国内外现状与切入点、研究目标、7 项研究内容、技术路线、8 项进度与完成情况 |
| 2. 阶段性成果 | ✅ 12 段（6 个小标题） | 数据资源与分析流程、预测结果、差异与特有肽、机制与实验方案、成果形式、阶段性结论 |
| 3. 公开发表学术论文情况 | ✅ 2 段 | 暂无发表，写明投稿与学位论文计划 |
| 4. 尚需完成的研究工作（包括内容、方案） | ✅ 18 段（5 个小标题） | 机制关联、抑菌实验、整理与投稿、问题与对策、后续安排 |
...
| skill | 内容 | 文件 |
|---|---|---|
| **可编辑 PPT 流水线** | 一份 JSON 大纲 → A/B/C 多套风格的原生可编辑 PPT + 自动校验（≥15 pt、不越界）+ 演讲备注；含大纲模板与"装到新项目"脚本 | `skills/editable-ppt-from-outline/`（`SKILL.md` / `README.md` / `outline.template.json` / `scripts/install.ps1`） |
...
| 位置 | 需要的信息 |
|---|---|
| ~~封面信息~~ | ✅ 已从 `sources/开题.docx` 填入（中期检查表 + 三版 PPT） |
