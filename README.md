...
## 🧭 新交付物：候选抗菌肽与 AD 病理关联解释版

这版专门纠正上一版“把 Aβ 结合讲成主线”的问题，直接解释：**深度学习从 AD 患者与健康人群肠道微生物组筛出的差异抗菌肽，如何与 AD 病理建立有证据边界的关联**。

| 文件 | 说明 |
|---|---|
| `deliverable/候选抗菌肽_AD病理关联解释版_20260926_1025_深度学习衔接/深度学习筛选肠道抗菌肽_AD病理关联机制解释版_20260926_1025.docx` | 详细解释版：预测 AMP、AD 差异 AMP、病理机制候选、保护/抑制亚类和因果肽的区别；主路径为 AMP—菌群—LPS/SCFA—屏障/炎症—脑病理；含差异分析、生态网络、MD/QM 优先级和答辩话术 |
| `deliverable/候选抗菌肽_AD病理关联解释版_20260926_1025_深度学习衔接/深度学习筛选肠道抗菌肽_AD病理关联机制解释版_20260926_1025.pptx` | 41 页零基础汇报版，每页带演讲备注，明确回答“找病理相关肽还是抑制剂相关肽” |
| 生成脚本 | `python3 code/make_amp_ad_pathology_linkage.py` |

> 核心口径：主对象是 **AD 相关差异微生物源 AMP**，不是预先寻找的 Aβ 抑制剂；保护性/抑制性肽只是功能亚类。
> “与 Aβ 结合”只说明分子相互作用，不能单独说明导致 AD。当前仓库没有实际候选 FASTA、丰度矩阵或 FDR 表，文档没有编造具体候选结果，AMP-C01 等只是填写模板。
...
| `deliverable/抗菌肽与AD关联机制说明.docx` | **机制解释补充材料**（老师指出机制没说清后新增）：逐条回答 8 个问题 + AChE–Aβ 分子动力学机制 + 三个可检验假设 + 证据强度分级 + 17 条参考文献；由 `code/make_mech_doc.py` 生成 |
...
- **机制补充页**（第 17—20 页，`code/make_ppt_mech.py`）：七环逻辑链、炎症—Aβ—抗菌肽正反馈、AChE–Aβ 机制与 H1—H3、证据强度分级；`central` 版 PPT 用 `--mechanism` 追加，中间版默认追加（共 12 页）。
