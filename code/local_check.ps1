...
# 3. zhongqi: the docx / pptx of THIS round must have arrived on this machine
#    byte for byte, and the OOXML packages must be structurally sound.
#    The real logic lives in code\check_deliverables.ps1 so that upgrading this
#    file (it is the skill's template) can never lose the repo's own checks.
#    Deep check with real Word / PowerPoint:  .\code\check_deliverables.ps1 -Com
$delivChk = '.\code\check_deliverables.ps1'
if (Test-Path -LiteralPath $delivChk) {
    try {
        $dOut = (& $delivChk 2>&1 | Out-String)
        $dCode = $LASTEXITCODE
        if ($dOut.Trim()) { Write-Output $dOut.TrimEnd() }
        if ($dCode -eq 0) {
            Write-Output '== accept 3: docx / pptx delivered intact (manifest + sha256 + OOXML structure)'
        } else {
            Write-Output ('[FAIL] accept 3: ' + $dCode + ' deliverable(s) did not verify - see the lines above')
            $fail = 1
        }
    } catch {
        Write-Output ('[FAIL] accept 3: check_deliverables.ps1 threw - ' + $_.Exception.Message)
        $fail = 1
    }
} else {
    Write-Output '[WARN] accept 3: code\check_deliverables.ps1 is missing - deliverable check skipped'
}
