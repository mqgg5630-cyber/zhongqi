...
**签字页 = 2. 导师综合评语（末尾“导师签字： 年 月 日”）+ Ⅲ.评议情况（检查小组成员名单 →
中期检查意见 → 是否同意参加预答辩 / 检查组长签字 / 培养单位盖章 / 年月日）** ——
导师签字和检查小组签字在**同一页**上，签一次就够。
...
1. **`中期.docx` / `中期新.docx` 里，签字页单独成完整的一页**
   ...
   也不会被拆到两页（估算：占一页 85%，还剩 107 pt）。前面几节以后怎么改都不影响它。

2. **再加一份只有签字页的一页文件：`中期检查表_签字页.docx`**
   从学校模板里原样裁出“2. 导师综合评语”到表格末尾（11 行逐字节与模板一致），A4 一页，
   ...
   签字页在估算分页里都落在最后一页（完整版第 10 页、中间版 1 与中间版 2 各第 6 页），
   且“导师签字”与“Ⅲ.评议情况 / 组长签字 / 单位盖章”在同一页。
