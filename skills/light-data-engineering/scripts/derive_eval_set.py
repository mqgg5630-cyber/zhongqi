#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""derive_eval_set.py — 据 research-plan 派生数据规格，从基础集生成鲁棒性/泛化/敏感性评测集。

research-plan 的实验矩阵里，鲁棒性/泛化/敏感性维度需要"派生评测集"：
在基础数据上做受控变换（加噪/缺失/跨域子集/扫参），用来测 ROB/GEN/SEN。
SKILL「回边」节说这些回 data-engineering 构建——本脚本把这条回边从"口头规格"变成
"规格 JSON → 可执行变换 → 派生集 + dataset_card 字段"（对齐 data_card 模板）。
...
def build_card_fields(name: str, base_name: str, t: dict, n_rows: int,
                      seed: int) -> dict:
    """为派生集生成 dataset_card 关键字段（对齐 data_card 模板 schema），回填用。"""
    return {
        "dataset_name": name,
        "domain": "derived-eval",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="据 research-plan 派生规格生成评测集 + dataset_card 字段")
    ...
    for r in res:
        ...
        print(f"[{r['transform']}] {csv_path} ({len(r['df'])} rows) + {card_path}",
              file=sys.stderr)
    print(f"生成 {len(res)} 个派生评测集到 {outdir}/；card_fields 可喂 croissant_export.py 出 Croissant 元数据。")
    return 0
