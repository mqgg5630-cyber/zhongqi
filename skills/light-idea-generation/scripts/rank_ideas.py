#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""rank_ideas.py — 候选 idea **分层组合裁定**（idea-generation 内部 triage，突破口不被性价比压杀）。
...
同一根轴上 PK。这是 idea-generation 内部 triage，不替代 idea-critique 八维严审，只决定"送哪几个、按什么结构送"。
...
不判 idea 真实价值（那靠 idea-critique），更不产生 idea（那靠人 + 底座模型）。分数从哪来要人填，垃圾进垃圾出。
...
def rank(items: list, top_k: int = 0) -> dict:
    ...
    return {"n": len(rows), "top_k": top_k,
            ...
            "note": ("分层组合裁定：moonshot/solid/safe 各赛道独立排序再 round-robin 组合，"
                     "突破口(高影响高代价)不被性价比压杀；这是 idea-generation 内部 triage，不替代 idea-critique 八维严审。"
                     "分数为主观快评，垃圾进垃圾出。")}


def render(res: dict) -> str:
    lc = res["lane_counts"]
    lines = [f"# 候选 idea 分层组合裁定（idea-generation triage，共 {res['n']} 条）", "",
    ...
    for ln in LANES:
        ...
        lines.append("")
    sl = res["shortlist"]
    lines += [f"## round-robin 组合 shortlist（{len(sl)} 条，三层交替取，送 idea-critique）",
              "| 送审序 | id | 道 | 标题 | 潜力 |", "|---|---|---|---|---|"]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="候选 idea 分层组合裁定（idea-generation 内部 triage）")
