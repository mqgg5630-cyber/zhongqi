#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""data_feasibility.py — 把"数据先行四问"汇成喂给 idea-generation/idea-critique 的标准结论卡。

data-engineering 对 research-plan/experiment-coding/result-analysis 的交接有 data_card.md / quality_report.md（重工件，给做实验用）。
但 data-engineering 对 idea-generation(提 idea)/idea-critique(审 idea) 的交接只有"四问结论"——idea 该不该立项，看的是
"数据够不够支撑"，不是完整数据卡。本脚本把四问收敛成一份轻量、可机检的落盘工件
`data_feasibility.md`，补上 data-engineering→idea 这条原本靠聊天传的单向挂载（**数据可行性前置于 idea 定稿，回边 2⊣3**）。
...
含 insufficient→insufficient（建议补采/改 idea，拦在 idea 前=回边 2⊣3，不进 idea 定稿）。
...
# 整体判定：四问最差档 → verdict
VERDICT = {
    "ok": ("USABLE", "✅", "数据基础充分，可进 idea-generation 提 idea"),
    "warn": ("USABLE_WITH_CAVEATS", "⚠️", "可进 idea-generation，但须在 idea 里正视下列保留项"),
    "insufficient": ("INSUFFICIENT", "🛑", "数据不足以支撑，先补采/补质或改 idea（拦在 idea 前，回边 2⊣3），不进 idea 定稿"),
}
...
def render(rep: dict) -> str:
    ...
    L.append("")
    L.append("> 交给 idea-generation(提 idea) / idea-critique(审 idea) 判 idea 是否有数据基础。"
    ...
    if rep["verdict_level"] == "insufficient":
        L.append("> 🛑 至少一问 insufficient：按 SKILL 核心原则，**拦在 idea 前（回边 2⊣3）**，先补采/补质或改 idea。")
    elif rep["verdict_level"] == "warn":
        L.append("> ⚠ 含保留项：可进 idea-generation，但 idea-critique 复核会针对 warn 项核 idea 是否正视该限制。")
    else:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="数据先行四问 → data_feasibility.md（喂 idea-generation/idea-critique，前置 2⊣3）")
    ...
    if args.out:
        ...
        print(md)
    # insufficient → 退出码 1，可在 pipeline 当"拦在 idea 前（2⊣3）"的闸门
