"""make_figs.py - publication-grade matplotlib templates (Light v2 / result-analysis).
...
def line_with_band(
    x, ys, ax=None, labels=None, xlabel="x", ylabel="y", title=None, cmap="viridis"
):
...
def demo(outdir=None):
    ...
    box_strip(
        ...
        ax=axes[0, 1],
        ylabel="accuracy",
        title="(b) Per-seed distribution",
    )
    x = np.arange(1, 11)
    curves = [
        rng.normal(np.linspace(0.6, base, 10), 0.01, (6, 10)) for base in (0.80, 0.86)
    ]
    line_with_band(
        x,
        curves,
    ...
    heatmap(
        ...
        xticks=["A", "B", "C"],
        yticks=["A", "B", "C"],
        title="(d) Norm. confusion",
        cbar_label="rate",
    )
    fig.suptitle(
        "Publication-grade figure template (viridis, CI, dpi=300)", fontsize=12
    )
