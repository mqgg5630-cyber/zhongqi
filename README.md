...
| `deliverable/中期检查表_签字页.docx` | **只有签字页的一页文件**（Ⅲ.评议情况 → 检查小组签字 → 检查意见 → 组长签字 / 培养单位盖章）：从学校模板原样裁出，逐行与模板一致，A4 一页；明天打印这一页去签字，正文改完后把签好的这页直接替换 / 装订到最后 |
...
如果这个文件夹已经被别的分支占了（切不回来），**双击 `reclone.cmd`** 另拉一个干净文件夹
...
| Ⅲ.评议情况 — 检查小组成员 | ⬜ 留空 | 模板原有行保留、成员姓名格全部留空，由检查小组现场填写（不预填名单）|
...
- **签字页单独成完整一页**：`Ⅲ.评议情况`那一行加了段前分页（`code/fill_docx.py` 新增 `page_break_before_row`，由 `code/build_ops.py` 自动带上），四份检查表里签字部分都落在最后一页；另出 `deliverable/中期检查表_签字页.docx` 供明天单独打印签字。分页用 `code/check_docx_layout.py` 估算核对、`code/preview_docx.py` 出图（无需装 Office）。
