...
def diagnose(df, target=None, corr_thresh=0.9, card_thresh=0.5, outlier_cap=20):
    ...
    if target and target in df.columns:
        ...
        ty = df[target]
        # 仅数值目标才判断“整数编码的低基数分类”。字符串分类标签（如 M/B）不能 float()；
        # 旧逻辑会在真实 WDBC 数据上 ValueError，虽已走分类分支判断仍提前崩溃。
        is_int_like = False
        if target in num_cols and ty.notna().any():
            is_int_like = bool(
                ty.dropna().apply(lambda v: float(v).is_integer()).all()
            )
...
def main():
    ...
    if args.selftest:
        ...
        # 字符串分类目标回归：旧版在 float("M") 处崩溃，真实 WDBC M/B 标签暴露。
        df["label_text"] = df["label"].map({0: "B", 1: "M"})
        fct = diagnose(df, target="label_text")
        leak_cols_text = {c for c, _ in fct["leakage_hint"]}
        assert "leak_num" in leak_cols_text, \
            f"字符串分类目标-数值特征泄漏检测失败: {leak_cols_text}"
        assert "leak_cat" in leak_cols_text, \
            f"字符串分类目标-类别特征泄漏检测失败: {leak_cols_text}"
        # 新增检测器断言
