...
中间版 2（--half）另按用户要求做减法：不再出现“与开题计划对照”页，机制页只留
七环逻辑链与 AChE–Aβ 动力学两页，删除“八个问题逐一作答”与“文献支撑一览”两页
（相关内容在《抗菌肽与AD关联机制说明》docx 里），共 11 页。
    ...
    if half:
        N.closing(prs, total, total,
            ...
            lines=[
                ...
                "以及三模型共识预测的候选抗菌肽名单。",
                "下一阶段：分阶段差异分析与宏蛋白组二次去重、AD 特异性特有抗菌肽筛选，"
                "随后开展机制关联分析与抑菌实验验证。",
                "时间安排：按开题计划推进，分析与验证所需数据与方案均已具备，"
                "学位论文与以第一作者投稿的 SCI 论文撰写同步准备。",
            ],
            note="中期完成的是数据与预测这一半，候选抗菌肽名单已经形成；"
                 "下一阶段推进分阶段差异分析、二次去重与 AD 特异性特有肽筛选，随后开展 AD 特有肽与 AD 的关联分析、"
        ...
        N.closing(prs, total, total,
            ...
            lines=[
                ...
                "宏蛋白组二次去重与 AD 特异性特有抗菌肽筛选。",
                "进行中：AD 特异性特有抗菌肽与 AD 的关联分析、候选抗菌肽的抑菌实验验证，"
    ...
    if half:
        N.figure_slide(prs, 3, total,
            ...
            reading="八项工作分为两半：数据资源、参考集、短肽库与三模型共识预测四项已完成；"
                    "分阶段差异分析、二次去重、AD 特异性特有肽筛选与关联验证四项在下一阶段推进。",
    ...
    if half:
        N.figure_slide(prs, 7, total,
            ...
            reading="按认知阶段比较候选抗菌肽的丰度与组成差异，再以宏蛋白组表达证据二次去重，"
                    "筛选 AD 组特有（AD 特异性）的抗菌肽；实施方案已确定。",
            note="下一阶段先做分阶段差异分析，再引入宏蛋白组表达证据做二次去重，剔除没有表达支持的序列，"
                 "筛选出 AD 组特有（AD 特异性）的抗菌肽清单。两项工作的方案已经确定。")
    else:
        N.figure_slide(prs, 7, total,
            headline="宏蛋白组表达证据二次去重后，得到 AD 特异性特有抗菌肽",
            figure="figD_宏蛋白组去重.png", layout="full",
            reading="先按认知阶段比较丰度与组成差异，再引入宏蛋白组表达证据，剔除“有预测、无表达”的序列，"
                    "得到 AD 组特有（AD 特异性）的抗菌肽清单。该步骤已完成。",
            note="序列层面去冗余解决重复；宏蛋白组表达证据解决“有预测、无表达”的假阳性。"
                 "二次去重后按组内共有、组间特比较，得到 AD 组特有（AD 特异性）的抗菌肽清单。")

    # 8 机制关联与抑菌实验 --------------------------------------------------
    N.figure_slide(prs, 8, total,
        headline="AD 特有肽与 AD 的关联分析与抑菌实验验证正在开展",
        figure="figE_机制关联.png", layout="rail",
        reading="AD 特有肽与 AD 的关联按致病方向展开：是否促进 Aβ 成核与聚集、是否经 AChE 外周阴离子位点"
                "推动成核、是否经免疫与炎症通路放大神经炎症；结论定位为线索发现。"
                "抑菌实验验证以代表性候选肽对大肠杆菌与金黄色葡萄球菌做纸片扩散法初筛，"
                "并以微量肉汤稀释法测定最低抑菌浓度。",
        note="机制关联参照乙酰胆碱酯酶—β-淀粉样肽复合物分子模拟研究的思路，方向按致病方向设定："
             "微生物来源的淀粉样蛋白与内毒素已有先例（FapC、CsgA 加速 Aβ 纤维化并在动物模型中加重病理，"
             "LPS 诱导斑块与 p-tau，LL-37 与 Aβ 形成异源团簇），所以考察的是 AD 特有肽是否促进 Aβ 成核与聚集、"
             "是否经 AChE 外周阴离子位点推动成核、是否经免疫与炎症通路放大神经炎症，"
             "而不是去证明它抑制 AD；结论定位为线索发现。抑菌实验验证选取有代表性的 AD 特有肽人工合成，"
             "以大肠杆菌与金黄色葡萄球菌为指示菌，先用纸片扩散法初筛，再用微量肉汤稀释法测最低抑菌浓度。")

    # 9 与开题计划对照（中间版 2 不输出这一页）-------------------------------
    if half:
        pass
    elif False:
...
def main(argv=None) -> int:
    ...
    # 中间版 2（--half）按用户要求做过减法：主体 8 页（无“与开题计划对照”页），
    # 机制页只留七环逻辑链（M1）与 AChE–Aβ 动力学（M3），末页是谢谢页 —— 共 11 页。
    mech_keys = ["m1", "m3"] if a.half else ["m1", "m3", "m5", "m6"]
    body = 8 if a.half else 9
    total = (body + len(mech_keys) + 1) if not a.no_mech else body + 1
    prs = build(meta, cover, total, half=a.half, with_final=a.no_mech)
    if not a.no_mech:                               # 机制页插在谢谢页之前
        MECH.append(prs, first_idx=body + 1, total=total, which="mid", keys=mech_keys)
