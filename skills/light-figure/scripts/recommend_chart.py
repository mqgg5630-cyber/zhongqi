...
的"建议工具/图型"字段，再交 validate_plan_card.py 做契约校验、程序化出图。
...
- 不涉及 AI 生图：只推荐"该用什么图型"，真图由 figure 程序化绘制（matplotlib/seaborn/ggplot/TikZ）。
...
CHART_KB = [
    ...
    {"chart": "box_violin", "zh": "箱线图/小提琴图",
     ...
     "why": "跨组分布比较——显示中位/四分位/离群;小提琴叠加密度。小样本叠原始散点(诚实展示分布)",
    ...
    {"chart": "heatmap", "zh": "热力图",
     ...
     "caution": "颜色编码精度低(只看模式不读精确值);固定 vmin-vmax(避免色标漂移误导);色盲安全色图"},
    ...
    {"chart": "error_bar_point", "zh": "点估计+误差棒",
     ...
     "needs": {"nominal", "quantitative"}, "max_series": 10,
     "why": "带不确定性的组间比较——点估计+CI/SD/SEM(注明误差棒类型与 n)",
...
def recommend(task: str, fields: list, n_series: int = 1, top_k: int = 5) -> dict:
    ...
    return {
        ...
        "note": ("启发式推荐(非最优化/非真值)，依据 Cleveland-McGill 感知精度+任务族经验先验；"
                 "最终选型须结合 claim 重要性/领域惯例/期刊偏好人工定夺。图由 figure 程序化绘制，不 AI 生图。"),
