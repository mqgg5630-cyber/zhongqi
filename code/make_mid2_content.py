...
分阶段差异分析、宏蛋白组去重与 AD 特异性特有肽筛选、AD 特有肽与 AD 的关联分析、抑菌实验验证全部列为
...
CUSTOM = {
    ...
    ("1", 35): "从完成结构上看，研究内容中的宏基因组数据处理与微生物基因组参考集构建、"
               ...
               "构成了课题的主体分析基础；分阶段差异分析、宏蛋白组去重与 AD 特异性特有抗菌肽筛选正在开展，"
    ...
    ("2", 6): "!（三）分阶段差异分析与 AD 特异性特有抗菌肽筛选（正在实施）",
    ("2", 7): "分阶段差异分析已进入实施：按 NC、SCS、SCD、MCI 与 AD 五个认知阶段比较候选抗菌肽的丰度与组成差异，"
              "识别可能随病程变化的候选抗菌肽。该环节与后续的二次去重衔接，分析结果将作为 AD 特异性特有抗菌肽筛选的输入。",
    ("2", 8): "宏蛋白组表达证据二次去重按计划推进：在序列去冗余的基础上，逐步引入蛋白层面的表达证据，"
              "剔除缺少表达支持的序列，再按组内共有、组间特比较，筛出 AD 组特有（AD 特异性）的抗菌肽。",
    # 机制关联方案：补上英文缩写写法（与 PPT 口径一致）
    ("2", 11): "已完成 AD 特异性特有抗菌肽与 AD 发病机制关联分析的技术方案设计：以分子对接与分子动力学模拟按致病方向考察"
    ...
    ("2", 14): "阶段性成果以数据资源、分析流程与候选名单为主要形式：已形成完整可重复的分析流程与技术文档；"
               "已形成候选抗菌肽名单及预测环节的统计信息；分阶段差异分析与 AD 特异性特有抗菌肽筛选正在实施。"
...
EXTRA = {
    ("1", 29): [
        "4. 其余各项（分阶段差异分析、宏蛋白组二次去重与 AD 特异性特有抗菌肽筛选、AD 特有肽与 AD 的关联分析、抑菌实验验证、"
        "结果整理与论文撰写）为中期之后的工作，实施方案与所需材料已确定。",
    ],
    ("4", 12): [
        "1. 预测结果可能存在假阳性。应对：以多模型共识判定提高候选集合的可信度，"
        "再以宏蛋白组表达证据二次去重，最后通过抑菌实验对代表性候选肽进行验证，形成层层递进的证据链。",
        "2. 分阶段差异分析与 AD 特异性特有抗菌肽筛选尚在实施。应对：按既定流程推进，先完成差异比较与表达证据过滤，"
        "再冻结 AD 特异性特有肽清单；后续关联分析与抑菌实验共用同一份清单，避免重复分析、控制工作量。",
...
ADVISOR = {
    "论文指导情况": (
        ...
        "目前数据处理与预测环节已完成，分阶段差异分析与 AD 特异性特有抗菌肽筛选正在推进，"
        "AD 特有肽与 AD 的关联分析与抑菌实验验证已排入后续计划。"
    ),
    "导师综合评语": (
