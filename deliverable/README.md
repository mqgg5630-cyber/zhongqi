...
**签字页 = Ⅲ.评议情况（检查小组成员名单 → 中期检查意见 → 是否同意参加预答辩 /
检查组长签字 / 培养单位盖章 / 2026年9月19日）** —— 检查小组签字与单位盖章在**同一页**上，
签一次就够；**2. 导师综合评语与“导师签字： 2026年9月19日”留在上一页**（2026-09-19 按用户意见移回）。

1. **`中期.docx`（以及同源的 `中期新.docx`）里，签字页单独成完整的一页**
   在“Ⅲ.评议情况”那一行加了**段前分页**（Word 的 `pageBreakBefore`，行内容、框线、
   列宽、行高一个字都没动）。于是签字部分整体落在**最后一页**上，不会跟前面的正文挤在一起，
   也不会被拆到两页（估算：占一页 51%，还剩 352 pt）。前面几节以后怎么改都不影响它；
   导师综合评语与导师签字仍在它前面那一页的下半部分。

2. **再加一份只有签字页的一页文件：`中期检查表_签字页.docx`**
   从填好的 `中期.docx` 里原样裁出“Ⅲ.评议情况”到表格末尾（10 行逐字节与整份表一致），A4 一页，
   ...
   版面图：`../results/docx_preview/三版签字页_Ⅲ.评议情况起_检查小组签字+单位盖章.png`、
   ...
   中间版 1 与中间版 2 各 7 页、签字页 = 第 7 页），
   且“检查小组成员 / 检查意见”与“组长签字 / 培养单位盖章”在同一页。
