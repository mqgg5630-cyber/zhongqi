...
# -*- coding: utf-8 -*-
"""
check_project_card.py —— .light/ 项目卡细粒度校验器 (Light v2 / light-memory-pm)

定位：校验每个项目 .light/ 目录下「项目记忆」文件的内容自洽。passport.py 管
DAG 台账（passport.yaml）结构；本脚本管 passport 不碰的 **项目卡 / 决策日志 /
版本史 / 交接卡** 的细粒度内容。四类内容校验：

  ABS_DATE     绝对日期格式：project_card.created 与 decision_log /
               version_history 每条目行首必须是 [YYYY-MM-DD]，禁相对日期
               (今天/昨天/上周/最近/N 天前…)。
  STAGE_ENUM   current_stage 的前导词必须落在 memory-pm 约定的 11 阶段枚举内。
...
用法：
  python check_project_card.py --project-dir <项目 .light/ 目录> [--handoff <.light/handoff>]
  python check_project_card.py --projects-root <含多个项目子目录的根> [--handoff ...]
...
# memory-pm 约定的 11 个 current_stage 阶段枚举（真相源；--readme 可覆盖同步）
DEFAULT_STAGES = [
...
def _e2e_selftest_dir():
    ...
    while True:
        if os.path.exists(os.path.join(root, "_shared", "__init__.py")):
            e2e = os.path.join(root, ".upgrade", "_e2e")
            os.makedirs(e2e, exist_ok=True)
            return e2e
        parent = os.path.dirname(root)
        if parent == root:
            e2e = os.path.join(os.getcwd(), ".upgrade", "_e2e")
            os.makedirs(e2e, exist_ok=True)
            return e2e
        root = parent
...
def parse_readme_stages(readme_path):
    """从 README/SKILL 解析 current_stage 枚举行，失败回退 DEFAULT_STAGES。"""
    ...
    if stage_val is None:
        ...
        if head not in stages:
            findings.append(_finding(
                ...
                f"current_stage 前导词 '{head}' 不在 memory-pm 11 阶段枚举内",
...
def run_audit(projects_root=None, project_dir=None, handoff_dir=None, readme=None):
    findings = []
    stages = parse_readme_stages(readme or (
        os.path.join(projects_root, "README.md") if projects_root else None))
    if project_dir:
...
def render_report(findings):
    out = ["=" * 64, ".light/ 项目卡细粒度校验报告 (light-memory-pm)",
...
"""
BAD_DECISION = """# 决策日志
- [2026-06-01] 决策A — 理由A — 来源 idea-generation
- 最近决定改方案，因为效果不好
- [2026-13-40] 决策C — 理由C — 来源 idea-critique
"""
BAD_VERSION = """# 版本记录
...
def selftest():
    ...
    tmp = tempfile.mkdtemp(prefix="mempm_selftest_", dir=_e2e_selftest_dir())
    rc = 1
    try:
        proj = os.path.join(tmp, "synth-bad")
        os.makedirs(proj)
        ...
        findings = run_audit(projects_root=tmp, handoff_dir=hd)
...
def main():
    ap = argparse.ArgumentParser(description=".light/ 项目卡细粒度校验器 (light-memory-pm)")
    ap.add_argument("--projects-root", dest="projects_root",
                    help="含多个项目子目录的根（扫 */ 每个项目的 project_card.md 等）")
    ap.add_argument("--project-dir", help="单个项目 .light/ 目录（含 project_card.md 等）")
    ap.add_argument("--handoff", help=".light/handoff 目录（校验 parent_session 链）")
    ap.add_argument("--readme", help="README/SKILL 路径（解析 current_stage 枚举；缺则用内置 11 阶段）")
    ap.add_argument("--selftest", action="store_true", help="离线合成自测，不依赖真实仓库")
    args = ap.parse_args()

    if args.selftest:
        return selftest()
    if not (args.projects_root or args.project_dir or args.handoff):
        return selftest()

    findings = run_audit(projects_root=args.projects_root, project_dir=args.project_dir,
