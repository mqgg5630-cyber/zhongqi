#!/usr/bin/env python3
"""figure_export.py — 投稿级图像导出工具 (Light / figure)
...
JOURNAL_SPECS = {
    ...
    "mdpi": {
        ...
        "1000dpi, 照片>=300dpi; TIFF/PNG/EPS。数据来源: figure "
        "references.md「出版商图宽硬规格核查表」MDPI 行(含 Animals/Agronomy/"
...
def _check_font_availability():
    ...
    if not ok:
        warning = (
            f"字体落空：rcParams 首选 {prefs[:3]} 不可用，实际渲染回退为 '{resolved_name}'"
...
def save_for_journal(
    ...
    **kwargs,
    ...
    - custom_width_mm: 逃生通道。journal="custom" 时直接用此物理栏宽(mm),
      ...
      数据须有来源(目标刊作者指南或实测记录), 禁止臆测。此时 dpi/formats 不给则取通用默认
    ...
    if j == "custom" or custom_width_mm is not None:
        ...
        if j not in JOURNAL_SPECS:
            raise ValueError(
                f"未知期刊 '{journal}', 可选: {list(JOURNAL_SPECS)} 或 'custom'+custom_width_mm"
            )
...
def check_figure_size(
    ...
    if limit is not None:
        ...
        if w_mm > limit + tol_mm:
            report["ok"] = False
            report["problems"].append(f"宽度 {w_mm:.1f}mm 超过上限 {limit}mm")
...
def check_export_compliance(path, journal, column="single", verbose=False):
    ...
    if j not in JOURNAL_SPECS:
        return {
            "ok": True,
            "skipped": f"custom/未知刊 {journal}，无合规规格可校验",
            "problems": [],
        }
    spec = JOURNAL_SPECS[j]
    problems, checks = [], {}
...
def _demo_and_selfcheck():
    ...
    for fmt in ("png", "pdf"):
        ...
        print(
            f"[demo] 导出后读回实测宽度({fmt})={rep_m['width_mm']}mm "
    ...
    print(
        "[demo] custom_width_mm=84 (中文刊逃生通道) ->",
        info5["width_mm"],
        "mm, check OK",
    )
