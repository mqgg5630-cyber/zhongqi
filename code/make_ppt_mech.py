...
def _m1(prs, idx, total):
    N.figure_slide(prs, idx, total,
        ...
        reading="① AD 与感染相关、② 炎症驱动抗菌肽与 Aβ 生成、③ Aβ 本身即抗菌肽；"
                "④ 抗菌活性可计算预测、⑤ 特异抗菌肽随阶段变化、⑥ 致病方向：促进 Aβ 聚集；"
                "⑦ 为什么 AD 更多与机制落点是本课题要回答的部分。",
...
def _m3(prs, idx, total):
    N.figure_slide(prs, idx, total,
        ...
        note="机制落点参照 AChE–Aβ 复合物的分子动力学研究：Aβ 对接进 PAS 后，复合物在 1 微秒模拟中保持稳定，"
             ...
             "推向更毒寡聚体、H3 在膜与小胶质水平放大炎症信号；验证路径是先用对接与动力学给出致病方向的"
             "优先序，再用 ThT 聚集动力学、电镜形态与细胞毒性实验给证据。")
...
def _m4(prs, idx, total):
    N.figure_slide(prs, idx, total,
        ...
        reading="抗菌肽与 Aβ 结构兼容，可交叉成核 [36][37]；微生物来源的 FapC 与 CsgA 是先例："
                "种子加速 Aβ 纤维化并在动物模型中加重病理 [47][48]；本课题按致病方向检验候选肽。",
