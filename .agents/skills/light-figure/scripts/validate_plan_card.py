#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""validate_plan_card.py — 图表规划卡的契约校验（把绘制阶段的打回前移到规划阶段）。

解析一张/一批图表规划卡（templates/figure_plan_card.md 的填充实例），校验规划层字段
是否与同技能 figure_export.py 的出版规格约束对齐，**在规划阶段就拦下
会被绘制阶段打回的卡**，而不是画完才发现栏宽键不存在：

  1. target_journal 必须命中 figure_export.py JOURNAL_SPECS 的键（nature/science/cell/...）
     或为 custom；custom 必须带 custom_width_mm（且为正数，禁臆测——须注明来源:目标刊作者指南/实测）。
  2. column 必须是该刊在 JOURNAL_SPECS 实有的档位（如 full 仅 science/mdpi、onehalf 仅
     plos/elsevier）；custom 时 column 省略。
  3. figure_id 必须形如 F<数字>/T<数字>，批量校验时须唯一（与论文占位 [图位 F1] 对齐）。
  4. source_card 必填（图的数据/模式来源指针(如 result-analysis 的 claim_evidence_table)，或 new_canonical_candidate 标记）。
...
def load_journal_columns() -> tuple[dict, bool]:
    """返回 ({journal: [valid columns]}, is_live)。优先从**同目录** figure_export.py 动态读
    (v2 planning+drawing 已合并为 figure 技能,figure_export 与本脚本同 scripts 目录)。"""
...
def validate_card(fields: dict, journal_cols: dict) -> list:
    ...
    if not fid:
        errors.append("figure_id 缺失（须形如 F1/T1，与论文占位对齐）")
    ...
    if not fields.get("source_card"):
        errors.append("source_card 必填（图的数据/模式来源指针，或 new_canonical_candidate 标记）")
    return errors
...
def _selftest() -> int:
    ...
    good = ("# 图表规划卡 · `F1`\n"
            ...
            "| **column** | `double` |\n"
            "| **source_card** | result-analysis:claim_evidence_table::bar_grouped |\n")
    f = parse_card(good)
    ...
    cust2 = parse_card("# 图表规划卡 · `F3`\n| **figure_id** | `F3` |\n"
                       "| **target_journal** | `custom` |\n"
                       "| **custom_width_mm** | 120 mm（来源:目标刊作者指南） |\n"
                       "| **source_card** | x |\n")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="图表规划卡契约校验（对齐 figure 出版规格约束）")
