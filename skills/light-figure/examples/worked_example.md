...
场景：奶山羊发情行为识别（示例项目），方法演示用，数值为合理示例非真实采集。
-->
...
| 字段 | 内容 |
|------|------|
| **figure_type** | `跨数据集主结果对比(分组柱+误差棒)`（选型可借 recommend_chart.py:comparison+nominal+quantitative→grouped_bar） |
| **purpose** | 多方法×多数据集的主指标比较 |
| **data_required** | 各方法各数据集 5 种子的 mean±std（来自 result-analysis claim_evidence_table.md） |
| **layout** | 单图，x=数据集分组，组内并列各方法 |
| **color_scheme** | Okabe-Ito 离散 5 色（4 baseline + 我方法），我方法用最深/强调色（color_palettes.py 取色） |
| **annotation_style** | 误差棒=±std 标注 n=5；y 轴含单位；显著性 * 标在我方法 vs 次优 |
| **possible_code_tool** | matplotlib 程序化生成（分组柱+误差棒），不 AI 生图 |
...
| **source_card（必填）** | `result-analysis:claim_evidence_table.md / Table 2` |
...
| **显著性/不确定性** | 带 ±std（n=5）；与 result-analysis 口径一致 |
...
| **source_card（必填）** | `result-analysis:claim_evidence_table.md / 消融部分` |
...
| **possible_code_tool** | **Draw.io**（diagram-as-code，`.drawio` 即 mxGraph XML 可版本控制，CLI `drawio --export` 出图）或 TikZ；程序化非 AI 生图 |
| **color_scheme** | 灰阶为主 + 创新模块 1 个强调色；色盲安全 |
| **target_journal / column** | `ieee` / `single`；矢量 PDF（框架图必矢量，缩放不糊） |
| **source_card（必填）** | `new_canonical_candidate`（全新框架图模式，无既有来源） |
...
- 每张卡 `source_card` 必填、数值追溯到 result-analysis，`target_journal/column` 取 JOURNAL_SPECS，交绘制前可跑 `validate_plan_card.py` 校验、`recommend_chart.py` 辅助选型、`visual_honesty_gate.py` 把诚实关。
