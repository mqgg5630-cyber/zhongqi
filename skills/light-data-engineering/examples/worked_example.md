...
## Step 4.5 — 四问结论卡（data_feasibility.py，交 idea-generation/idea-critique）

把 Step 1-4 收敛成给 idea-generation/idea-critique 的标准工件（区别于给 research-plan/experiment-coding 做实验的 data_card/quality_report）：
...
- **idea-generation 读它**：verdict 非 INSUFFICIENT 才提 idea；warn 项（如"发情类规模待 power analysis"）写进 idea 约束。
- **idea-critique 读它**：核 idea 自报"数据够"是否与该卡一致，不一致按封顶处理（data 维）。
- 若把发情类改到 40 条，Q3 变 insufficient → 整体 INSUFFICIENT、退出码 1 → **拦在 idea 前（回边 2⊣3）**：补数据/改 idea。

> 落位：三件套（data_card.md / quality_report.md / data_feasibility.md）+ leak_findings.json 落 `.light/` 标准名；登记 passport 由 memory-pm `pm.py` / orchestrator `passport.py append-stage` 负责（artifacts 路径是 consistency 回扫的权威清单）。

---

## Step 5 — 数据卡（data_card_template.md）

填 `assets/data_card_template.md`（对齐 HF Dataset Card / Datasheets / Croissant）。关键节示意：
...
## 四问总结（喂给 idea-generation/idea-critique）
...
> 这套走查产出 quality_report.md + data_card.md + **data_feasibility.md** + leak_findings.json：前两件交 research-plan/experiment-coding/result-analysis 做实验，**data_feasibility.md 交 idea-generation/idea-critique 判 idea 是否有数据基础**（`.light/` 标准交接；数据可行性前置于 idea 定稿=回边 2⊣3）。每步脚本均纯本地零网络、带 selftest。
