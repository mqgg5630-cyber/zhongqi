#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""make_ppt_mech_master.py - 抗菌肽与AD关联机制说明 单独PPT，ppt-master 精美版

使用 ppt-master 的 SVG→原生 DrawingML 通路，产出三版精美风格：
  - F 深色科技风（近黑底+青色+网格）
  - G 学术期刊风（白底+期刊红+细线）
  - M 机制专属·深蓝金（深蓝底+金色强调，更精美）

内容来源：docs/ppt_outline_mech.json（15页）

用法：
    python code/make_ppt_mech_master.py --style F
    python code/make_ppt_mech_master.py --style G
    python code/make_ppt_mech_master.py --style M
    python code/make_ppt_mech_master.py --style all
"""
...
OUTLINE = ROOT / "docs" / "ppt_outline_mech.json"
...
def figure_page(st,s,idx,total,content_top):
    ...
    panel_h = avail_h if not body else int(avail_h*0.58)
    ...
    if body:
        ...
        remaining = 600 - body_top
        max_lines = 3 if has_takeaway else 4
        if remaining < 70:
            max_lines = 2
        # 缩窄到1000px，给ppt-master文本框测量留冗余，避免shape leaves canvas
        xml,_=block(MARGIN, body_top, body, px=20, fill=st["body"], width=1000, lh=1.35, family=st["body_family"], max_lines=max_lines)
        out.append(xml)
    return out
...
def bullets_page(st,s,idx,total,content_top):
    ...
    for b in bullets:
        ...
        if rest:
            ...
            xml,ny=block(MARGIN+22+text_w(tag,20)+12, y, rest, px=20, fill=st["body"], width=1000-text_w(tag,20)-34, lh=1.35, family=st["body_family"], max_lines=2)
        else:
            out.append(rect(MARGIN, y-12, 8, 8, st["accent"]))
            xml,ny=block(MARGIN+22, y, b, px=20, fill=st["body"], width=1000, lh=1.35, family=st["body_family"], max_lines=3)
        out.append(xml)
        y=ny+16
        if y>bottom:
            break
    if callout:
        ...
        xml,_=block(MARGIN+22, top+64, callout["text"], px=20, fill=st["body"], width=1000, lh=1.32, family=st["body_family"], max_lines=2)
        out.append(xml)
    return out
...
def cards_page(st,s,idx,total,content_top):
    ...
    for b in bullets:
        if y>520:
            break
        xml,ny=block(MARGIN,y,"· "+b,px=20,fill=st["body"],width=620,lh=1.32,family=st["body_family"],max_lines=2)
        out.append(xml)
        y=ny+12
    cy=content_top+4
    ch=82
    for label,desc,tone in cards:
        ...
        out.append(t(784,cy+30,label,px=20,fill=col,family=st["body_family"],weight="bold"))
        xml,_=block(784,cy+54,desc,px=20,fill=st["body"],width=400,lh=1.22,family=st["body_family"],max_lines=2)
        out.append(xml)
        cy+=ch+10
    return out
...
def steps_page(st,s,idx,total,content_top):
    ...
    for k,(tag,text) in enumerate(steps):
        if y+70>610:
            break
        out.append(rect(MARGIN,y,6,68,st["accent"] if k%2==0 else st["accent2"]))
        out.append(t(MARGIN+22,y+28,tag,px=20,fill=st["title"],family=st["body_family"],weight="bold"))
        xml,_=block(MARGIN+22,y+52,text,px=20,fill=st["body"],width=1000,lh=1.32,family=st["body_family"],max_lines=2)
        out.append(xml)
        y+=68+16
    return out
...
def toc_page(st,s,idx,total,content_top):
    ...
    for k,item in enumerate(items):
        ...
        y=content_top+20+row*92
        out.append(t(x,y,f"{k+1:02d}",px=28,fill=st["accent"],family=st["kicker_family"],weight="bold"))
        xml,_=block(x+56,y,item,px=20,fill=st["body"],width=col_w-56,lh=1.28,family=st["body_family"],max_lines=2)
        out.append(xml)
        out.append(line(x,y+42,x+col_w,y+42,st["rule"],1))
    return out
...
def cover(st,s,idx,total,meta):
    ...
    for i,m in enumerate([x for x in metas if x]):
        out.append(t(MARGIN, y+48+i*36, m, px=20, fill=st["body"], family=st["body_family"]))
...
def render(style_key, outline):
    ...
    meta=outline["meta"]
    total=len(slides)
    proj=BUILD / f"svgproj_mech_{style_key}"
...
- audience: 生命科学学院答辩评委，关注机制与文献
- objective: 全面解释抗菌肽与AD关联的8个问题，补充38篇文献
- core_message: 抗菌肽与Aβ同属先天免疫，双刃剑经AChE-PAS与炎症通路形成正反馈
## mode
- mode: custom
- mode_behavior: 机制解释专用，按8问+文献链展开
## visual_style
- visual_style: custom
- visual_style_behavior: {st['name']}精美版，{'深色科技+金色强调' if style_key=='M' else '深色科技' if style_key=='F' else '期刊风'}
## colors
- primary: {st['accent']}
- accent: {st['accent2']}
- background: {st['bg']}
...
""", encoding="utf-8")
    ...
    out=OUT_DIR / f"抗菌肽与AD机制_{style_key}_{st['name']}.pptx"
