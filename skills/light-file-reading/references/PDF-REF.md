...
配套脚本：`scripts/pdf_ops.py`（已自检通过：triage/meta/text/tables/merge/split/rotate）。
...
```bash
python scripts/pdf_ops.py triage document.pdf
```

- `born_digital`：优先原生文本；结构简单用 pdfplumber/pypdf，论文需章节/引文高保真时再选 GROBID/Docling。
- `mixed`：文本页走原生抽取，`route.ocr_or_visual_pages` 逐页走 OCR 或视觉补读，不能只看整份抽取字数。
- `scanned`：整份走本地 OCR/宿主视觉；敏感材料不上传云端。
- `sparse_or_unknown`：先渲染抽样页核验，可能是封面、图页、加密、损坏或阈值不适配。

这是输入画像 advisory，不是读取质量证明。长文档应解析一次，先保存目录/页级地图，再按定位展开目标页；
完整研究工作流与资源 access 分级见 [`reading-resource-map.md`](reading-resource-map.md)。
