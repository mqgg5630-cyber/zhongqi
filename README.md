...
| `results/docx_preview/完整版/page-09.png` | 签字页那一页的版面图（第 9 页 = 最后页；`完整版/全部页.png` 为 9 页全图） |
...
| `deliverable/中期检查表_填写内容_无抑菌实验.md` | **送审版正文草稿**（`code/make_noexp_content.py` 从上面的草稿自动派生：整段删掉抑菌实验小节，并改掉研究内容 / 进度 / 阶段性成果 / 尚需完成工作 / 问题与对策 / 后续安排 / 导师评语里的相关说法，共删 8 行、改 30 处）；`deliverable/中期.docx` 就是用这份 md 生成的 |
| `deliverable/中期.docx` | **填好的中期检查表（送审版：不含抑菌实验）**（含封面：文绍华 / 2024110316 / 生命科学学院 / 生物学 / 指导教师 申亮；含 Ⅱ.导师指导情况；检查小组成员已按学院给的名单填好：组长 李向阳（教授），组员 江婷婷（副教授）、孙杰（教授）、高洪伟（教授），专业均为生物化学与分子生物学，单位均为鲁东大学生命科学学院），在学校空白模板上就地填写，格式与模板完全一致（脚本逐格校验通过） |
| `deliverable/中期新.docx` | 第二个文件名（"两版中期"用）：**仍按原稿生成（含抑菌实验）**，与上面的送审版内容不同；若要两版一致，把送审版那条 `fill_docx.py` 命令的 `--out` 换成它即可 |
| `deliverable/中期检查表_签字页.docx` | **只有签字页的一页文件**（2. 导师综合评语 / 导师签字 2026年9月19日 → Ⅲ.评议情况：检查小组成员 → 中期检查意见 → 是否同意参加预答辩 / 检查组长签字 / 培养单位盖章）：从填好的 `deliverable/中期.docx` 原样裁出，11 行逐行与整份表一致，A4 一页；明天打印这一页去签字（**导师签字与检查小组签字同页**），正文改完后把签好的这页直接替换 / 装订到最后 |
...
| 2. 导师综合评语 — 导师签字 | ✅ 已填日期 | 评语正文由脚本写入（草稿，供导师改），末尾“导师签字： 2026年9月19日”已填日期、手写签名留给导师现场签；这一行**与下面的 Ⅲ.评议情况组装在同一页**（整份表最后一页） |
| Ⅲ.评议情况 — 检查小组成员 / 组长签字 / 单位盖章 | ✅ 已填 / ⬜ 待签 | 成员已按学院名单填好：组长 李向阳（教授），组员 江婷婷（副教授）、孙杰（教授）、高洪伟（教授），专业均为生物化学与分子生物学，单位均为鲁东大学生命科学学院；模板备用的第 15 行留空。签名与盖章现场办，日期已填 2026年9月19日；与导师签字同一页 |
...
- **签字页单独成完整一页**：分页加在 `2. 导师综合评语` 那一行（`code/fill_docx.py` 的 `page_break_before_row`，由 `code/build_ops.py` 自动带上），于是导师评语 / 导师签字 / 检查小组成员 / 检查意见 / 是否同意预答辩 / 组长签字 / 培养单位盖章整体落在**最后一页**、不会被拆页 —— 用户 2026-09-19 指定导师评语与签名部分“组装在一起”。另出 `deliverable/中期检查表_签字页.docx`（11 行）供明天单独打印签字。分页用 `code/check_docx_layout.py` 估算核对（含“签字部分同页”检查）、`code/preview_docx.py` 出图（无需装 Office）。
...
- **送审版（`deliverable/中期.docx`）去掉抑菌实验**：用户指出这一版不要实验部分，所以正文草稿不直接改，而是用 `code/make_noexp_content.py` 从 `deliverable/中期检查表_填写内容.md` 派生 `deliverable/中期检查表_填写内容_无抑菌实验.md`（删掉"（二）候选抗菌肽的抑菌实验验证"整小节与研究内容第 7 项等 8 行，并改掉研究内容 / 技术路线 / 进度 / 阶段性成果 / 尚需完成的工作 / 问题与对策 / 后续工作安排 / 导师评语里的实验说法 30 处，小节编号自动理顺），再做 `build_ops.py` → `fill_docx.py`；`中间版 1 / 中间版 2 / 中期新.docx` 仍按原稿生成、保留实验内容。生成的 md 里 11 个实验关键词（抑菌 / 纸片扩散 / 肉汤稀释 / 指示菌 / 最低抑菌浓度 / 阳性对照 / 阴性对照 / 人工合成 / 活性验证 / 大肠杆菌 / 金黄色葡萄球菌）复核为 0 处，`code/check_consistency.py` 也会持续核对"送审版无实验、其余版本保留"与评语文本。
