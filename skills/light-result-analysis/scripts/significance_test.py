...
Light v2 result-analysis reusable function library. Bundles the three
...
the co-located stats_tests.py when importable, else falls back to local
...
# ---- reuse verified primitives from the co-located stats_tests.py ------------
# v2: stats_tests.py 与本文件同目录(港自 v1 code_assets),治 v1 硬编码 ../../../code_assets 之脆。
...
def _selftest() -> int:
    ...
    print(
        f"diff_CI    mine=({ci[0]:.6f},{ci[1]:.6f}) scipy=({rci.low:.6f},{rci.high:.6f})"
    )
    ...
    print(
        f"diff_CI_eq mine=({ci_eq[0]:.6f},{ci_eq[1]:.6f}) scipy=({rci_eq.low:.6f},{rci_eq.high:.6f})"
    )
    ...
    print(
        f"boot_CI    mine=({bci[0]:.5f},{bci[1]:.5f}) scipy=({sb.confidence_interval.low:.5f},{sb.confidence_interval.high:.5f})"
    )
    ok &= (
        abs(bci[0] - sb.confidence_interval.low) < 5e-3
        and abs(bci[1] - sb.confidence_interval.high) < 5e-3
    )
    ...
    print(
        f"compare_two p mine={r['p']:.6f} scipy={rp:.6f} | d={r['cohens_d']:.3f} ({r['effect']})"
    )
    ok &= abs(r["p"] - rp) < 1e-9
    ...
    try:
        ...
        print(
            f"BH-FDR     q match statsmodels: {np.allclose(q, q2)} | reject match: {np.array_equal(rej, r2)}"
        )
    ...
    print(
        f"DeLong     auc_a={dl['auc_a']:.3f} auc_b={dl['auc_b']:.3f} "
