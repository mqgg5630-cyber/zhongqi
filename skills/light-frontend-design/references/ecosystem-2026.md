...
## 设计 token 工程化（用户项目内部单源→多端;v2 无 Light 中央 SSOT）
...
- **v2 诚实定位**：单源→多端的工程化方法论用于**用户自己的前端项目内部**。**v1 曾把它绑到 `db05/design_tokens.template.json`(称由 a07 consistency 维护)——一手核 v2 无 db05、无任何 design_tokens 文件、consistency(a07) 只管 claims/术语/方法/指标一致性、不管视觉 token**。故 v2 frontend **不绑 Light 中央视觉库**;若同项目也用 figure 出论文图，配色/字体由用户/总控**人工对齐**(无中央文件可同取)。

## 当前版本快照

> ⚠ **这是时间快照,不是真相**——版本会腐,**装前必当天 `npm view` 复检**(下方命令)。
> 实证:2026-06-10→2026-06-22 仅 12 天,`tailwindcss` 4.3.0→**4.3.1**、`lucide-react` 1.17.0→**1.21.0** 已漂(8 个复核包里 2 个变)。

npm 检查（前 8 行 `npm view` **真核于 2026-06-22**;末 2 行仍是 2026-06-10 快照、未于 06-22 复核 → 标注）：

| 包 | 版本 | last_checked |
|---|---:|---|
| `next` | 16.2.9 | 2026-06-22 |
| `react` | 19.2.7 | 2026-06-22 |
| `tailwindcss` | 4.3.1 | 2026-06-22（06-10 时为 4.3.0） |
| `shadcn` | 4.11.0 | 2026-06-22 |
| `motion` | 12.40.0 | 2026-06-22 |
| `gsap` | 3.15.0 | 2026-06-22 |
| `lucide-react` | 1.21.0 | 2026-06-22（06-10 时为 1.17.0） |
| `vite` | 8.0.16 | 2026-06-22 |
| `@radix-ui/react-slot` | 1.2.5 | 2026-06-10（未于 06-22 复核） |
| `@vitejs/plugin-react` | 6.0.2 | 2026-06-10（未于 06-22 复核） |
