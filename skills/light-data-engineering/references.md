...
## Vaex（已淘汰，仅存档）
> ⚠ Vaex 已进入**维护模式**（原核心贡献者退出；2026-06 一手核：仍有零星发布，vaex-core 4.19.0 于 2025-09，非 2023 即停）——**新项目优先 DuckDB（SQL）/ polars streaming**（更活跃、生态更广）。引擎选型按数据规模/场景在本笔记内对照（无单独 tool-selection 技能）。以下为历史方法存档。
...
- 【与一致性指标衔接】cleanlab 解决"单一标注与模型预测不一致"的标签错误检测；多标注者之间的一致性（IAA）用 sklearn `cohen_kappa_score`（`weights="quadratic"` 做加权 κ）/ statsmodels `fleiss_kappa`（≥3 标注者）/ pingouin·statsmodels 的 ICC(2,1)。两者互补：IAA 评标注流程整体可靠度，cleanlab 定位具体可疑样本。
