...
CUSTOM = {
    ...
    ("2", 11): "已完成特有抗菌肽与 AD 发病机制关联分析的技术方案设计：以分子对接与分子动力学模拟考察候选肽与"
               "β-淀粉样肽（Aβ）、乙酰胆碱酯酶（AChE）外周阴离子位点的结合情况与复合物稳定性，"
               "并结合文献比对与功能注释分析其可能涉及的免疫与炎症通路。相关分析按方案在下一阶段实施，"
               "候选肽的序列与结构准备、参数设置与流程搭建等前期工作已完成。",
