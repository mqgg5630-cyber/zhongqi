...
# ── 挂接共享语义相似度契约 (_shared/semantic_sim.py) ──────────────
...
# 规范 bootstrap（_shared/README.md）：向上走目录树找仓库根，治硬编码 parents[N]
# 在三 harness/全局安装/任意嵌套下必断的脆（v1 此处硬编码 parents[2]，v2 仓库根上移一层已不成立）。
...
def run(query: str, per_page: int = 10, offline_sample: bool = False,
    ...
    offline = False
    recs: list[dict] = []
    http_codes: dict = {}   # 各源真实 HTTP 码(v2 新增):供 domain_map 覆盖度诚实门,不臆测 DOAJ 是否真到
    if not offline_sample:
        ...
        http_codes = {"OpenAlex": oa_code, "Crossref": cr_code}
...
def _selftest() -> int:
    ...
    _sleep = record_sleep  # 不真睡，记录退避秒数
    calls = {"n": 0}
