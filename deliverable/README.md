...
- `中期.docx`：在**你的模板原件**（`sources/中期.docx`）上就地填入内容后的成品（含 Ⅱ.导师指导情况；检查小组成员已填：组长 李向阳，组员 江婷婷、孙杰、高洪伟，单位均为鲁东大学生命科学学院）。
- `中期新.docx`：第二个文件名，用于"两版中期"同时提交/对比；**仍按原稿生成（含抑菌实验）**，与上面的送审版内容不同
  （若要一致：`python code/fill_docx.py --ops build/ops_中期_无抑菌实验.json --out deliverable/中期新.docx`）。
...
- **日期**：封面填表日期、导师签字、检查组长签字 / 培养单位盖章这 3 处空档由脚本一次填成 **2026年9月19日**
  （`code/fill_docx.py` 的 `fill_date_blanks`；模板里这几处在 Word 内容控件里，只换文字、字体格式不动）。

## 1.1 签字页单独成页（明天要签的那一页）

检查表正文还要改，但签字这一页明天就要签，所以做了两件事。
**签字页 = 2. 导师综合评语 / 导师签字 2026年9月19日 + Ⅲ.评议情况（检查小组成员名单 →
中期检查意见 → 是否同意参加预答辩 / 检查组长签字 / 培养单位盖章）** —— 导师签字与检查小组签字在
**同一页**上（2026-09-19 按用户意见组装在一起），签一次就够。

1. **`中期.docx`（以及同源的 `中期新.docx`）里，签字页单独成完整的一页**
   在“2. 导师综合评语”那一行加了**段前分页**（Word 的 `pageBreakBefore`，行内容、框线、
   列宽、行高一个字都没动）。于是签字部分（导师评语 / 导师签字 + 检查小组成员 / 检查意见 /
   组长签字 / 单位盖章）整体落在**最后一页**上，不会跟前面的正文挤在一起，也不会被拆到两页
   （送审版估算：占一页 83%，还剩 121 pt）。前面几节以后怎么改都不影响它。

2. **再加一份只有签字页的一页文件：`中期检查表_签字页.docx`**
   从填好的 `中期.docx` 里原样裁出“2. 导师综合评语”到表格末尾（11 行逐字节与整份表一致），A4 一页，
   ...
   版面图：`../results/docx_preview/三版签字页_导师签字+检查小组签字同页.png`、
   `../results/docx_preview/完整版_全部页缩略图.png`、`../results/docx_preview/签字页_单独文件_一页.png`。
   签字页在估算分页里都落在最后一页（送审版 `deliverable/中期.docx` 共 9 页、签字页 = 第 9 页；
   `中期新.docx` 共 10 页、签字页 = 第 10 页；中间版 1 与中间版 2 各 6 页、签字页 = 第 6 页），
   且“导师签字”与“检查小组成员 / 检查意见 / 组长签字 / 培养单位盖章”在同一页。
