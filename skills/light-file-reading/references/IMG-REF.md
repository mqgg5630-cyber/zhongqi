...
> 处理他人图片时按 key 名引用敏感字段、不回显具体值（隐私/权属风险归 research-ethics 红线门）。
...
- 反提得到的数据/公式/表格 → 填入 `assets/understanding-note.template.md` 的「可复用内容」。
- 要把反提数据重新画成图 → 走 figure 技能，**程序化重绘，绝不 AI 生成**。
- 反提的引用/文献信息 → citation 技能核验。
