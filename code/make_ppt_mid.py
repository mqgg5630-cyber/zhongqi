...
def build(meta: dict, cover: dict, total: int = 14, half: bool = False,
    ...
    N.figure_slide(prs, 8, total,
        ...
        reading="机制关联按致病方向展开：候选肽是否促进 Aβ 成核与聚集、是否经 AChE 外周阴离子位点"
                "推动成核、是否经免疫与炎症通路放大神经炎症；结论定位为线索发现与优先序。"
        ...
        note="机制关联参照乙酰胆碱酯酶—β-淀粉样肽复合物分子模拟研究的思路，方向按致病方向设定："
             ...
             "LPS 诱导斑块与 p-tau，LL-37 与 Aβ 形成异源团簇），所以考察的是候选肽是否促进 Aβ 成核与聚集、"
             "是否经 AChE 外周阴离子位点推动成核、是否经免疫与炎症通路放大神经炎症，"
             "而不是去证明它抑制 AD；结论定位为线索发现。抑菌实验验证选取有代表性的候选抗菌肽人工合成，"
