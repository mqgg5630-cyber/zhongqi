...
【Light 接法】Tika 只能作为 text/metadata 证据；不得用 `/tika` 成功来证明 reading order、layout、table grid
或公式保真。若把 Tika 放进 `adapter_runs`，`channels_claimed` 只能含 `text/metadata`，否则
`reading_contract.py` 会以 `ADAPTER_CAPABILITY_MISMATCH` 阻断。
