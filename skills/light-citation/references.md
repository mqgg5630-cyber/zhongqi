...
> Round 2 执行入口已迁到 [`citation-resource-map.md`](citation-resource-map.md)，
> canonical schema 见 [`references/registry_contract.md`](references/registry_contract.md)。
> 本文件只保留 API、格式和参考管理器的按需细节，不是执行顺序或状态真相源。
> 当前纪律：注册源 + 第二独立字段源才 `CONFIRMED`；DOI RA 明确不存在且
> Crossref/DataCite 都 404 才 `CONFIRMED-MISSING`；429/5xx/网络失败保留
> `UNAVAILABLE/UNRESOLVED`。OpenAlex 需免费 key，只是可选增强。
...
> 最后实测日期：2026-06-06；**接入口径 2026-06-21 复核**：Crossref 免 key（mailto 可选，~50 req/s，2025-12-01 调整）·
> OpenCitations 免 key（180 req/min/IP，v2.2.0）· doi.org 内容协商免 key · Semantic Scholar 免 key（5000/5min 共享池）·
> Unpaywall 免 key（email 必填，10 万/天）· **OpenAlex 自 2026-02-13 起需免费 key、官方停用 polite pool/mailto**
> （$1/天免费额度；唯一口径见 literature-search references「OpenAlex 接入真相源」）。故 citation 以**真正免 key 的
> DOI RA/Crossref/DataCite/PubMed/doi.org/OpenCitations 为免 key 路径，
> OpenAlex 降可选增强（守硬约束「不要求注册账号」）。
...
【是什么】开放学术图谱（接替已关停的 Microsoft Academic Graph），覆盖 works/authors/sources/institutions/concepts/topics 全实体，适合大规模检索与计量。**自 2026-02-13 起接入需免费 API key（官方停用 polite pool）**，key/限流/计费的唯一口径见 literature-search references「OpenAlex 接入真相源」节。

【✅ 已验证 2026-06-06；接入口径以 literature-search 真相源为准】
...
- 注：上述 6-06 实测为匿名（仅 mailto）放行，属 key 强制过渡期现象；**现行政策需免费 key**，生产代码不硬依赖匿名。citation 以 DOI RA + Crossref/DataCite 注册源和 PubMed/S2 独立字段源为免 key 主路径，OpenAlex 仅作可选增强。
...
- **接入**：OpenAlex 自 2026-02-13 需免费 `api_key`（官方停用 polite pool/mailto 池）。限流/计费具体口径见 literature-search references「OpenAlex 接入真相源」节，本处不复写数字。
...
【被 literature-search 消费】literature-search 的「单篇深读协议」用本节做"检索到→合法拿全文"的衔接：只取 OA 版本深读，闭源回退元数据+摘要级精读并声明（改本节端点须顾及 literature-search 深读节的指针）。
...
> `doi_to_any.py` / `verify_refs.py` 均以 DOI 为入口。**知网（CNKI）、万方收录的大量中文文献无 DOI**，整条核验在 DOI 链路里缺失。本节是无 DOI 中文文献的**核验执行方**：把 paper-writing（含原 paper-drafting）诚信门「无 DOI 中文文献核对协议」比对出的题录，固化成字段齐全、可排版的规范 .bib 条目。
> **双向声明（避免单向挂载）**：paper-writing 是写作时的**拦截方**（落笔即比对、核不到标"待核查"）；本节是**核验执行方**（题录→.bib 字段齐全性核对）。两方比对的字段集一致：题录三字段（题名 / 作者+单位 / 刊名+年卷期页），就是下面 .bib 必须齐全的字段。
...
- [ ] 与 paper-writing 留痕台账对账：该条已有 `[核对日期] … 三字段一致` 记录（核不到的不进 .bib，留"待核查"）。
...
| 需求 | 首选 | 备选 |
|---|---|---|
| 验证 DOI/标题/作者/年份真实 | DOI RA + Crossref/DataCite 注册源 | PubMed、Semantic Scholar、OpenAlex（需 key） |
