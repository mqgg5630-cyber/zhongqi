...
def std_flow_slide(prs, idx, total, progress="full"):
    ...
    if progress == "half":
        note = ("这页说明整体流程：从测序数据到候选抗菌肽清单，共十二步。"
                "工具链是 fastp 与 KneadData 质控去宿主、MEGAHIT 组装、MetaBAT2、MaxBin2 与 CONCOCT "
                "三个分箱算法分别分箱，再用 metaWRAP 提纯、dRep 去冗余、EMBOSS getorf 预测 sORF、"
                "三模型一致阳性判定、"
        ...
        note = ("这页说明整体流程：从测序数据到候选抗菌肽清单，共十二步，输入输出都可追溯。"
             ...
             "三个分箱算法分别分箱，再用 metaWRAP 提纯，dRep 按 95% ANI 去冗余得到参考基因组集，"
...
def final_closing(prs, total: int) -> None:
    ...
    closing(prs, total, total,
        ...
        lines=[
            "确认事实：数据资源、短肽库、三模型共识预测、分阶段差异分析与 AD 特异性特有抗菌肽筛选均已完成。",
...
def build(outline: dict, total: int = 24, *, with_final: bool = True) -> Presentation:
    ...
    claim_slide(prs, 3, total,
        ...
        support=[
            ...
            "统计与表达层：按认知阶段比较差异，再以宏蛋白组表达证据二次去重，筛出 AD 特异性特有抗菌肽。",
            "功能层：考察这些 AD 特有肽与 AD 的关联，并做抑菌实验验证。",
        ],
        aside=["当前状态：",
               "前四层中的前三层已完成，功能层正在实施；",
               "整体进度与开题计划一致。"],
    ...
    figure_slide(prs, 10, total,
        ...
        note="在序列去冗余的基础上，引入宏蛋白组的表达证据做二次去重，只保留在蛋白层面真实存在、可检出的抗菌肽；"
             "再按组内共有、组间特比较，得到 AD 组特有（AD 特异性）的抗菌肽清单，这一步已完成。")

    figure_slide(prs, 11, total,
        headline="AD 特有肽与 AD 的关联：Aβ、AChE 与炎症通路",
        figure="figE_机制关联.png", layout="rail",
        reading="借鉴乙酰胆碱酯酶—β-淀粉样肽复合物分子模拟研究的思路，按致病方向考察 AD 特异性特有肽："
                "是否促进 Aβ 成核与聚集、是否经 AChE 外周阴离子位点推动成核、是否经免疫与炎症通路"
                "放大神经炎症；结论定位为线索发现。",
        note="机制关联参照乙酰胆碱酯酶—β-淀粉样肽复合物分子模拟研究的思路，方向按致病方向设定："
             ...
             "因此分析的是 AD 特异性特有肽是否促进 Aβ 成核与聚集、是否经 AChE 外周阴离子位点推动成核、"
             "是否经免疫与炎症通路放大神经炎症，而不是去证明它抑制 AD；结论定位为线索发现。")
    ...
    closing(prs, 16, total,
        ...
        lines=[
            "机制关联分析：完成 AD 特异性特有抗菌肽与 Aβ 聚集、AChE 结合及炎症通路的关联分析。",
            "抑菌实验验证：完成候选抗菌肽的合成与抑菌实验，获得抑菌活性初筛结果。",
            "结果整理与论文撰写：汇总 AD 特有肽、机制关联与验证结果，完成学位论文与投稿论文准备。",
        ],
    ...
    closing(prs, 17, total,
        ...
        lines=[
            "预期成果：一套可复用的分析流程、AD 特异性特有的候选抗菌肽清单、AD 特有肽与 AD 关联的分析结果、"
