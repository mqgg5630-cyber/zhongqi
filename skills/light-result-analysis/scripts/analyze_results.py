"""analyze_results.py - one-command result-table analysis (Light v2 / result-analysis).
...
Reuses verified stats from the co-located significance_test.py.
"""
...
# 挂接共享地基契约(证据强度):q/效应量/CI → 措辞档机械映射,产 evidence_strength.json,
# 供 paper-writing/citation/figure/consistency/research-ethics 卡"措辞不强于证据"(证据强度桥产出端)。
# 规范 bootstrap(_shared/README.md):向上走目录树找含 _shared 包的仓库根,治 v1 硬编码 ../../_shared 之脆。
import pathlib
...
try:
    from _shared.evidence_contract import build_evidence_json, save as _ev_save  # noqa: E402
...
def group_test(df, group, metric):
    ...
    if all_normal:
        ...
        out["posthoc_note"] = (
            "non-normal multi-group: pairwise Mann-Whitney below, BH-FDR corrected（如需严格事后检验用 Dunn/scikit-posthocs）"
        )
    # 小样本警告：Shapiro 在 n 小时功效低，"判正态"不可靠
    min_n = min(len(x) for x in data)
    if min_n < 10:
        out["small_n_warning"] = (
            f"最小组 n={min_n}<10：Shapiro 正态性检验功效低，'判正态'不可靠，"
...
def paired_pairwise(df, group, metric, pair_key):
    ...
    for g1, g2 in itertools.combinations(groups, 2):
        ...
        if n < 2:
            rec.update(
                test="skipped", reason=f"only {n} shared {pair_key} value(s)", p=None
            )
            comps.append(rec)
            continue
...
def analyze_metric(df, group, metric, pair_key=None):
    out = {
        "metric": metric,
        "eda": eda_summary(df, group, metric),
        "omnibus": group_test(df, group, metric),
        "pairwise": pairwise(df, group, metric),
    }
    if pair_key:
        out["paired"] = paired_pairwise(df, group, metric, pair_key)
        if out["paired"].get("available"):
            out["inference_mode"] = "paired"
            out["unpaired_advisory"] = (
                "pairwise/omnibus 的独立样本结果仅保留作敏感性描述；"
                "claim_evidence_table 与 evidence_strength 只采用 paired comparisons，"
                "避免把同一 seed/fold/sample 重复当独立样本。"
            )
        else:
            out["inference_mode"] = "paired_unavailable"
    else:
        out["inference_mode"] = "independent"
    return out


def primary_comparisons(res):
    """Return the declared-design comparisons used for claims/evidence.

    If a valid pairing key was supplied, independent tests remain visible only
    as an advisory sensitivity view and MUST NOT create duplicate claims.
    """
    paired = res.get("paired")
    if paired and paired.get("available"):
        return "paired", paired.get("comparisons", [])
    if "paired" in res:
        # A requested paired design that cannot be formed is not permission to
        # silently reinterpret the same rows as independent observations.
        return "unavailable", []
    return "independent", res.get("pairwise", [])
...
def slice_analysis(df, group, metric, slice_by, min_n=5, pair_key=None):
    ...
    if slice_by not in df.columns:
        return {
            "available": False,
            "reason": f"slice column '{slice_by}' not in columns",
        }
    slices = []
...
def run(
    ...
    outdir,
    pair_key=None,
    emit_claim_table_flag=False,
    slice_by=None,
    emit_evidence_flag=False,
):
    df = pd.read_csv(csv_path)
    if group not in df.columns:
        raise SystemExit(
            f"group column '{group}' not in CSV columns {list(df.columns)}"
        )
    if not metrics:
        metrics = [
            c for c in df.columns if c != group and pd.api.types.is_numeric_dtype(df[c])
        ]
...
def _render_md(report):
    L = [
        f"# Result Analysis: `{os.path.basename(report['csv'])}`",
        "",
    ...
    for res in report["results"]:
        ...
        for r in res["eda"]:
            L.append(
                f"| {r['group']} | {r['n']} | {r['mean']:.4f} | {r['std']:.4f} "
        ...
        if ob.get("p") is not None:
            ...
            L.append(
                f"**Omnibus test**: {ob['test']} -> p = {ob['p']:.4g} ({sig}). "
        ...
        if ob.get("posthoc"):
            ...
            for t in ob["posthoc"]:
                L.append(
                    f"| {t['group1']} | {t['group2']} | {t['meandiff']:.4f} | {t['p_adj']:.4g} | {t['reject']} |"
                )
        ...
        for c in res["pairwise"]:
            ci = c["diff_ci95"]
            L.append(
                f"| {c['group1']} | {c['group2']} | {c['test']} | {c['p']:.4g} | "
        ...
        if paired and paired.get("available"):
            ...
            for c in paired["comparisons"]:
                if c.get("p") is None:
                    L.append(
                        f"| {c['group1']} | {c['group2']} | {c['n_pairs']} | "
                        f"{c['test']} | — | — | — | — | {c.get('reason', '')} |"
                    )
                    continue
                ci = c.get("diff_ci95", [float("nan"), float("nan")])
                L.append(
                    f"| {c['group1']} | {c['group2']} | {c['n_pairs']} | {c['test']} | "
...
def emit_claim_table(report: dict) -> str:
    """把每个比较(claim)连到证据字段(检验/p/q/d/CI/n)，落盘标准工件
    claim_evidence_table.md。交 paper-writing(写作)/figure(绘图)时每条论断都能追到统计证据，杜绝
    '正文说更好但拿不出检验'。只收录有 p 值的比较；显著性以 BH-FDR 后 q 为准。"""
    L = [
        f"# Claim–Evidence Table: `{os.path.basename(report['csv'])}`",
        "",
    ...
    for res in report["results"]:
        m = res["metric"]
        mode, comparisons = primary_comparisons(res)
        for c in comparisons:
            ...
            if mode == "paired":
                paired = res["paired"]
                L.append(
                    f"| {m} | {c['group1']} vs {c['group2']} | {c['test']}(paired) | {c['p']:.4g} | "
                ...
                L.append(
                    f"| {m} | {c['group1']} vs {c['group2']} | {c['test']} | {c['p']:.4g} | "
    ...
    L.append("")
    L.append(
        f"_共 {n_claim} 条 claim。交 paper-writing(写作)/figure(绘图)；显著性看 q 不看 p；"
...
def emit_evidence_strength(report: dict) -> dict:
    """把每个比较转成 light.evidence_strength.v1（证据强度桥产出端，挂接 _shared/evidence_contract）。

    每条 claim 经 grade_evidence(q,效应量,CI,n) 定档，附 allowed/forbidden 措辞，
    供 paper-writing 草稿 lint、citation 引用、consistency/research-ethics 机械卡"措辞不强于证据"。
    ...
    ev["source"] = "result-analysis:analyze_results"
    return ev
...
def _synth_csv(path, seed=0):
    ...
    for method, (mu, sd) in specs.items():
        for _ in range(8):
            ...
            rows.append(
                {"method": method, "seed": _, "acc": round(acc, 4), "f1": round(f1, 4)}
            )
...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="light_analyze_results_") as tmp:
        ...
        for res in report["results"]:
            ...
            assert res["omnibus"].get("test") in {
                "anova_oneway",
                "welch_anova",
                "kruskal_wallis",
            }, res["omnibus"]
        ...
        assert "seed" not in rep2["metrics"], (
            "pair key must not be analyzed as a metric"
        )
        ...
        rep3, _, _ = run(
            odd, "method", ["acc"], os.path.join(tmp, "o3"), pair_key="seed"
        )
        skipped = [
            c
            for c in rep3["results"][0]["paired"]["comparisons"]
            if c["test"] == "skipped"
        ]
        ...
        rep4, _, _ = run(
            csv_path, "method", ["acc"], os.path.join(tmp, "o4"), pair_key="no_such_col"
        )
        assert rep4["results"][0]["paired"]["available"] is False, rep4
        assert primary_comparisons(rep4["results"][0]) == ("unavailable", []), (
            "missing paired key must not silently fall back to independent claims"
        )
        ...
        assert "vs" in ctext and "paired by seed" in ctext, (
            "应按声明设计产 paired 比较行"
        )
        assert "| independent |" not in ctext, (
            "paired 设计不得再产独立样本 duplicate claim"
        )
        ...
        for sub in ("groupA", "groupB"):
            n_per = (
                8 if sub == "groupA" else 2
            )  # groupB 故意小 n → 触发 small_n_warning
        ...
        scsv2 = os.path.join(tmp, "slice.csv")
        pd.DataFrame(srows2).to_csv(scsv2, index=False)
        reps2, _, _ = run(
            scsv2, "method", ["acc"], os.path.join(tmp, "oslice"), slice_by="subgroup"
        )
        ...
        wcsv = os.path.join(tmp, "wvar.csv")
        ...
        F, d1, d2, p = _welch_anova(
            [
                np.array([1.0, 2, 3, 4, 5]),
                np.array([2.0, 4, 6, 8, 10]),
        ...
        scsv = os.path.join(tmp, "small.csv")
        pd.DataFrame(srows).to_csv(scsv, index=False)
        reps, _, _ = run(scsv, "method", ["v"], os.path.join(tmp, "os"))
        assert "small_n_warning" in reps["results"][0]["omnibus"], reps["results"][0][
            "omnibus"
        ]

        # evidence_strength.json 生成（证据强度桥产出端，挂接 _shared/evidence_contract）
        if _HAS_EVIDENCE:
            ...
            assert len(ev["claims"]) == 3, (
                "3 组 paired 设计应只有 3 条主比较，不能另产 3 条独立 duplicate"
            )
            assert all(c["claim_id"].endswith(":paired") for c in ev["claims"])
            for cl in ev["claims"]:
                assert cl["evidence_grade"] in {"strong", "moderate", "weak", "none"}, (
                    cl
                )
                assert "allowed_verbs" in cl and "hedge_required" in cl, cl
            print(
                f"[selftest] evidence_strength: {len(ev['claims'])} claims, "
                f"grades={sorted(set(c['evidence_grade'] for c in ev['claims']))}"
            )
        else:
            print(
                "[selftest] WARN: evidence_contract 契约不可用，跳过 evidence_strength 测试"
            )
    print(
        "[selftest] PASS analyze_results (independent + paired + skip + missing-key + claim-table + slice + welch-anova + small-n + evidence)"
    )
    return 0
...
def main():
    ...
    ap.add_argument(
        "--emit-claim-table",
        action="store_true",
        help="额外产出 claim_evidence_table.md（每个比较↔证据字段，交 paper-writing/figure 的标准工件）",
    )
    ap.add_argument(
        "--emit-evidence",
        action="store_true",
        help="额外产出 evidence_strength.json（挂接 _shared 证据强度契约，"
        "q/效应量/CI→措辞档，交 paper-writing 草稿 lint/citation/consistency/research-ethics）",
    )
    ap.add_argument(
        "--slice-by",
        default=None,
    ...
    ap.add_argument(
        "--selftest", action="store_true", help="run synthetic offline self-test"
    )
    a = ap.parse_args()
    ...
    if not a.csv:
        ...
        a.group = "method"
        a.metric = a.metric or ["acc", "f1"]
        print(f"[demo] generated synthetic CSV -> {a.csv}")
    outdir = a.outdir or os.path.join(
        os.path.dirname(os.path.abspath(a.csv)), "analysis_out"
    )
    report, jp, mp = run(
        a.csv,
        a.group,
        a.metric,
        outdir,
        pair_key=a.paired_by,
        emit_claim_table_flag=a.emit_claim_table,
        slice_by=a.slice_by,
    ...
    for res in report["results"]:
        ob = res["omnibus"]
        print(
            f"  [{res['metric']}] {ob.get('test')} p={ob.get('p')}  "
