"""leakage_overfit_check.py - train/val/test gap + data-leakage screen (Light v2 / result-analysis).
    ...
    if train_s is not None and val_s is not None:
        ...
        if g > GAP_OVERFIT:
            flags.append(
                {
                    "type": "overfit",
                    "severity": "high" if g > 2 * GAP_OVERFIT else "medium",
                    "detail": f"train-val gap {g:.3f} > {GAP_OVERFIT}",
                    "value": float(g),
                }
            )
    if val_s is not None and test_s is not None:
        ...
        if g > GAP_SHIFT:
            flags.append(
                {
                    "type": "val_test_shift",
                    "severity": "high" if g > 2 * GAP_SHIFT else "medium",
                    "detail": f"val-test gap {g:.3f} > {GAP_SHIFT} (shift or val leakage)",
                    "value": float(g),
                }
            )
    return flags
...
def _deepchecks_note(train, test, target):
    try:
        ...
        return {
            "available": True,
            "note": "deepchecks importable; run data_integrity().run(Dataset(...)) for the full suite",
        }
    except Exception:
        return {
            "available": False,
            "note": "deepchecks not installed -> using built-in numpy/pandas checks only (degraded mode, still functional)",
        }
...
def run(
    ...
    train = (
        ...
        else (pd.read_csv(train_csv) if train_csv else None)
    )
    test = (
        test_df
        if test_df is not None
        else (pd.read_csv(test_csv) if test_csv else None)
    )
    flags = []
...
def _synth():
    ...
    const = np.ones(n)
    train = pd.DataFrame(
        {"x1": x1, "x2": x2, "leak_feature": leak, "dead_col": const, "y": y}
    )
    # test: fresh rows + 5 rows copied verbatim from train (exact-match leakage)
    m = 80
    xt1 = rng.normal(0, 1, m)
    xt2 = rng.normal(0, 1, m)
...
def main():
    ...
    ap.add_argument(
        "--selftest", action="store_true", help="run synthetic leakage self-test"
    )
    a = ap.parse_args()
    ...
    if not a.train:
        print(
            "[demo] no --train given: synthetic data with a planted leak + dup rows + dead col"
        )
        tr, te = _synth()
        report = run(target="y", scores=(0.99, 0.85, 0.71), train_df=tr, test_df=te)
    else:
        report = run(
            a.train, a.test, a.target, (a.train_score, a.val_score, a.test_score)
        )

    out = a.out or os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "leakage_report.json"
    )
