...
def _selftest() -> int:
    ...
    try:
        def fake_verify_arxiv(source):
            return 200, {
                "arxiv_id": "2401.01234",
                "title": "A preprint on goats",
            }

        verify_arxiv = fake_verify_arxiv
        ...
        def unavailable_arxiv(source):
            return 0, None

        def fake_reverse_lookup(title, max_n=3):
            return [{"doi": "10.1/cand", "title": title, "title_sim": 0.9}]

        verify_arxiv = unavailable_arxiv
        crossref_reverse_lookup = fake_reverse_lookup
