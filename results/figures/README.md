...
| 文件 | 用途 | 是否用于本次中期材料 |
|---|---|---|
| `figA_研究思路总览.png`、`figB_三模型预测.png`、`figC_分阶段差异分析.png`、`figD_宏蛋白组去重.png`、`figE_机制关联.png`、`figF_抑菌实验方案.png`、`figG_进度甘特.png` | **当前中期答辩 PPT 用图**：只表达研究思路与完成状态，不含技术细节与数值 | ✅ 用于 `deliverable/中期答辩_A—E_*.pptx`（五版共用同一套图） |
