...
| `deliverable/中期.docx` | **填好的中期检查表**（含封面：文绍华 / 2024110316 / 生命科学学院 / 生物学 / 指导教师 申亮；含 Ⅱ.导师指导情况；检查小组成员格按模板留空，由检查小组现场填写），在学校空白模板上就地填写，格式与模板完全一致（脚本逐格校验通过） |
| `deliverable/中期新.docx` | 同一内容的第二个文件名（"两版中期"用），与 `中期.docx` 由同一脚本生成、内容一致 |
| `deliverable/中期检查表_签字页.docx` | **只有签字页的一页文件**（2. 导师综合评语 + 导师签字 → Ⅲ.评议情况 → 检查小组签字 → 检查意见 → 组长签字 / 培养单位盖章）：从学校模板原样裁出，11 行逐行与模板一致，A4 一页；明天打印这一页去签字（导师签字与检查小组签字同页），正文改完后把签好的这页直接替换 / 装订到最后 |
| `deliverable/抗菌肽与AD关联机制说明.docx` | **机制解释补充材料**（老师指出机制没说清后新增）：逐条回答 8 个问题 + **按致病方向**验证候选肽能否推动 AD（AChE–Aβ 分子动力学 + 交叉成核机制）+ 三个可检验假设 + 证据强度分级 + **58 条参考文献**；正文源 `docs/机制说明_正文.md`，由 `code/make_mech_doc.py` 生成 |
...
| 2. 导师综合评语 — 导师签字 | ⬜ 留空 | 评语正文由脚本写入（草稿，供导师改），末尾“导师签字： 年 月 日”留空给导师现场签；这一行与下面的 Ⅲ.评议情况同属**签字页**，在整份表最后一页 |
| Ⅲ.评议情况 — 检查小组成员 | ⬜ 留空 | 模板原有行保留、成员姓名格全部留空，由检查小组现场填写（不预填名单）；与导师签字同一页 |
...
- **签字页单独成完整一页**：`2. 导师综合评语`（末尾即“导师签字”）那一行加了段前分页（`code/fill_docx.py` 新增 `page_break_before_row`，由 `code/build_ops.py` 自动带上），于是导师签字与检查小组签字（Ⅲ.评议情况 / 检查意见 / 组长签字 / 单位盖章）落在**同一页**、且是整份表最后一页；另出 `deliverable/中期检查表_签字页.docx`（11 行，含两处签字）供明天单独打印签字。分页用 `code/check_docx_layout.py` 估算核对（含“两处签字同页”检查）、`code/preview_docx.py` 出图（无需装 Office）。
- **机制补充页**（最终版/H 版第 18—23 页 / 中间版与中间版 2 第 10—13 页，`code/make_ppt_mech.py`）：七环逻辑链、炎症—Aβ—抗菌肽正反馈、**动力学如何验证候选肽的致病方向**（H1 结合 PAS/344—361 → 促进成核）、交叉成核、八问作答表、文献一览表；插在结束页之前。
...
- **结束页在最后一页**：最终版/H 版 24 页、中间版与中间版 2 各 14 页，"谢谢各位老师"都在最后一页；机制页插在结束页之前。
