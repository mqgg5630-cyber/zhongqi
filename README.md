...
| 文件 | 说明 |
|---|---|
| `deliverable/中期.docx` | **填好的中期检查表**，在你上传的学校空白模板上就地填写，页面设置、表格框线、字体字号与模板完全一致（已用脚本逐项校验） |
| `deliverable/中期答辩.pptx` | **16 页中期答辩 PPT**，16:9，全篇最小字号 **15 pt**，图内文字经投影缩放核算同样 ≥15 pt |
| `deliverable/中期检查表_填写内容.md` | 报告正文草稿（纯文本，最方便修改；改完可一键重新生成 docx） |
| `results/ppt_preview/预览_第*-*页.png` | 4 张 PPT 版式预览图，不用装 Office 也能核对 |
| `results/figures/*.png` | 8 张报告/PPT 插图（技术路线、产出规模、队列设计、结果图、P 值、甘特图、模型、下一步） |

> **需要你确认的两件事**
> 1. 中期检查表**封面**（姓名、学号、研究生类型、培养单位、学科专业、研究方向、指导教师）与 PPT 封面的【姓名】【学号】【导师】由你手工填写 —— 填表说明要求"除签名外需打印填写"，我没有代填。
> 2. 报告里"整体进度约 65%""模型完成度约 35%"是**按任务清单折算的估算值**（依据：8 项任务中 5 项 100%、1 项进行中、2 项未开始），请按实际调整。所有数字的出处见 `docs/数据台账.md`。
